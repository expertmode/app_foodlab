...
export function productPrompt(p, kind) {
    ...
    const subtitle = clean(p.subTitle).replace(/\n/g, ' ');
    ...
    if (kind === 'img_bg')
        return `Hyperrealistic editorial food photography, extreme close-up macro shot, top-down overhead flat-lay (camera pointing straight down at 90 degrees). Subject: the surface of "${title}" — show only the texture filling the entire frame.${subtitle ? ` Highlighted variants/ingredients: ${subtitle}.` : ''}${ctx ? ` Product description: ${ctx}` : ''} Render the actual specific ingredients mentioned visually as visible elements (e.g. if "ervilha" appears, show pea grains; if "microalgas" appears, show green specks; if "cereais" appears, show grain seeds). Ultra-sharp micro detail: visible fibers, grains, droplets, herbs, glossy moisture. Vivid natural colors faithful to the ingredients. Frame is filled 100% edge-to-edge with the texture only — no full object visible, no plate, no edges. Panoramic horizontal composition. Soft natural directional lighting, shallow depth of field. No text, no logos, no packaging, no human, no tilt, no perspective angle.`;
    if (kind === 'bottom_img')
        return `artistic close-up of a unique detail of "${title}"${subtitle ? ` (${subtitle})` : ''} — texture, main ingredient, water droplet — clean composition, ${STYLE_BASE}`;
    return '';
}
